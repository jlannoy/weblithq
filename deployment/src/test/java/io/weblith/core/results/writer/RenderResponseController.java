...
@NotLogged
@Controller
public class RenderResponseController {
    ...
    @Get
    public Result text() {
        return new TextResult("hello weblith");
    }

    @Get
    public Result file() throws URISyntaxException {
        URL path = this.getClass().getResource("/" + MY_TXT_FILE);
    ...
    public Result unknownTypeFile() throws URISyntaxException {
        URL path = this.getClass().getResource("/" + MY_UNKNOWN_TYPE_FILE);
        Objects.requireNonNull(path);
        return new StreamResult(new File(path.toURI()));
    }
