...
In the previous example, Webltith uses Class, methods and parameters names as default values for building HTTP routes. But you can always be more specific by providing a string value to the corresponding annotation.
...
Result classes have some shortcut methods to configure the future response, like adding Cookies and changing the charset (`UTF-8` by default). You can also define your own custom `Result` classes if needed. Here are some useful examples...
...
That last one, the `Redirect` result, is an important core concept of Weblith.

* Use distinctly `GET` and `POST` methods like you would do in a RESTful application.
* Successful POST actions should never directly render results.
* GET methods should never be used to modify things.
* All your POST actions will be CSRF-protected (unless marked with the `@NotCsrfProtected` annotation). 

That means all your `POST` methods should end up with a redirection to a `GET` method. The `Redirect` result will help you this way, by allowing you to specify a human-readable result message (being a message key - check the I18N chapter - or the text message to display).
