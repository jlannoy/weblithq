...
While you can configure yourself your JAR-RS `Response` objects, Weblith offers an handy way (for a server-side rendered web application) to manage the results of your routes. It can be done via different types of `Result` objects : 
...
Result classes have some shortcut methods to configure the future response, like adding Cookies and changing the charset (`UTF-8` by default). You can also define your own custom `Result` classes if needed. Here are some usefull examples...
