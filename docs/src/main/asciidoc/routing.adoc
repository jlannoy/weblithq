...
Result classes have some shortcut methods to configure the future response, like adding Cookies and changing the charset (`UTF-8` by default). You can also define your own custom `Result` classes if needed, by using advanced interfaces like `ConfigureResponse`, `RenderResponse` and `AutomaticCachingPolicy`. Here are some useful examples of the predefined result classes.
...
That means all the `POST` annotated methods should end up with a redirection to a `GET` annotated method. The `Redirect` result will help you this way, by allowing you to specify a human-readable result message (being a message key - check the I18N chapter - or the text message to display).
